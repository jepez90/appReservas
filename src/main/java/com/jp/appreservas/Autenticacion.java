	...
	public boolean isUserLogued() {
		if (sesion.getAttribute("isUserLogedNow")==null) {//sesion nueva
			...
			if((boolean)sesion.getAttribute("isUserLogedNow")) {//sesion ya establecida usuario no logueado
				if (sesion.getAttribute("UserLoguedNow")==null) {
					return false;
				}
				else {
					return true;
	...
	public int userEdit(HttpServletRequest request) {
		...
		if(parametros.containsKey("editUser")) {
			...
			if(parametros.containsKey("Nombres")) {
				if(Pattern.matches("\\D{5,100}", parametros.get("Nombres")[0])) usuario.setNombre(parametros.get("Nombres")[0].toUpperCase());
				else return 4;
			}else {
				return 4;
			}
			if(parametros.containsKey("p")) {
				if(parametros.get("p")[0].length()>=Usuario.MinCaracteresForP) usuario.setPass(parametros.get("p")[0]);
				else return 4;
			}
			else {
				return 4;
			...
			Usuario usuario = new Usuario();
			if(parametros.containsKey("u")) {
				if(Pattern.matches("[A-z0-9]{"+ Usuario.MinCaracteresForU + ",50}", parametros.get("u")[0])) usuario.setUserName(parametros.get("u")[0].trim());
				else return 4;
			}else {
				return 4;
			}
			if(parametros.containsKey("Cedula")) {
				if(Pattern.matches("\\d{7,10}", parametros.get("Cedula")[0]))usuario.setCedula(Long.parseLong(parametros.get("Cedula")[0].trim()));
				else return 4;
			}else {
				return 4;
			}
			if(parametros.containsKey("Nombres")) {
				if(Pattern.matches("\\D{5,100}", parametros.get("Nombres")[0])) usuario.setNombre(parametros.get("Nombres")[0].trim());
				else return 4;
			}else {
				return 4;
			}
